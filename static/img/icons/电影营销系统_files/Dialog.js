...
define(
    function (require) {
        ...
        function onEnterOk(okBtn) {
            ...
            // $('.ui-dialog').attr('tabindex', 1).focus().on('keypress.dialog', function (e) {
            //     var e = e ? e : (window.event ? window.event : null);
            //     if (e.keyCode === 13) {
            //         okBtn.fire('click');
            //     }
        ...
        Dialog.confirm = function (args) {
            ...
            var title = lib.encodeHTML(args.title) || '提示';
            var OK_TEXT = lib.encodeHTML(args.okText) || '确定';
            var CANCEL_TEXT = lib.encodeHTML(args.cancelText) || '取消';
            var content = (args.raw ? args.content : lib.encodeHTML(args.content)) || '';
            ...
            okBtn.setContent(OK_TEXT);
            cancelBtn.setContent(CANCEL_TEXT);
        ...
        Dialog.alert = function (args) {
            ...
            var title = (args.raw ? args.title : lib.encodeHTML(args.title)) || '提示';

            var content = (args.raw ? args.content : lib.encodeHTML(args.content)) || '';

            var successDialogClassName = args.isSuccessDialog ? ' ui-dialog-icon-success' : '';

            var noIconClassName = args.noIcon ? ' ui-dialog-icon-noIcon' : '';
            ...
            var tpl = [
                '<div class="ui-dialog-icon ui-dialog-icon-${type}${successDialogClassName}${noIconClassName}"></div>',
                '<div class="ui-dialog-text">${content}</div>'
            ].join('');
            ...
            dialog.setContent(
                lib.format(tpl, {
                    type: type,
                    content: content,
                    successDialogClassName: successDialogClassName,
            ...
            dialog.setFoot(''
                ...
                + dialogId + '-' + okPrefix + '; skin:spring;width:50;height:26;">'
                + Dialog.OK_TEXT
                + '</div>'
            );
