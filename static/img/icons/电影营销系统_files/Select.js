 ...
 *
 * extra
 * search 参数，select控件中如果有search为true的属性则可以进行本地搜索
...
define(
    function (require) {
        ...
        Select.prototype.getDisplayHTML = function (item) {
            ...
            var data = {
                text: this.search ? text : lib.encodeHTML(text),
                value: lib.encodeHTML(item.value)
            };
        ...
        function hideLayer(select, layer) {
            ...
            if (select.search && layer) {
                var selectList = layer.getElementsByTagName('li');
                var len = selectList.length;
                while (len--) {
                    lib.removeClass(selectList[len], 'hide');
                }
            }
        }
        ...
        function openLayer(select) {
            var layer = getSelectionLayer(select);
            if (!layer) {
                layer = helper.layer.create('ol');
                layer.id = helper.getId(select, 'layer');
                layer.className = helper.getPartClasses(select, 'layer').join(' ');
        ...
        Select.prototype.initStructure = function () {
            ...
            var textHolder;
            if (!this.search) {
                textHolder = '<span class="textHolder"></span>';
            }
            else {
                textHolder = '<input type="text" id="inputTextHolder" class="textHolder inputTextHolder" ${width} />';
            }
            var html = [
                textHolder,
            ...
            if (this.search) {
                helper.addDOMEvent(this, this.main.getElementsByClassName('inputTextHolder')[0], 'keyup', searchValue);
            }

        };

        Select.prototype.toggleLayer = function () {
            toggleLayer.call(this);
        };
        ...
        function updateValue(select) {
            ...
            var hidden;
            if (!select.search) {
                hidden = select.main.getElementsByTagName('input')[0];
            }
            else {
                hidden = select.main.getElementsByTagName('input')[1];
            }
            ...
            var displayText = select.getDisplayHTML(selectedItem);
            if (!select.search) {
                textHolder = select.main.getElementsByTagName('span')[0];
                textHolder.innerHTML = displayText;
            }
            else {
                textHolder = select.main.getElementsByTagName('input')[0];
                textHolder.value = displayText;
            }
        ...
        function searchValue(e) {
            ...
            while (len--) {
                lib.removeClass(selectList[len], 'hide');
                if (!reg.test(datasource[len].name)) {
                    lib.addClass(selectList[len], 'hide');
                }
            }
        }
        ...
        Select.prototype.disable = function () {
            ...
            if (this.search) {
                this.main.getElementsByClassName('inputTextHolder')[0].disabled = true;
            }
        };
        ...
        Select.prototype.enable = function () {
            ...
            if (this.search) {
                this.main.getElementsByClassName('inputTextHolder')[0].disabled = false;
            }
        };
