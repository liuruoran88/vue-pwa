...
define(
    function (require) {
        ...
        function initResizeHandler(table) {
            ...
            helper.addDOMEvent(table, window, 'scroll', resizeHandler);
            helper.addDOMEvent(table, $('body #main')[0], 'click', function () {
                resizeHandler(1);
            });
        }
        ...
        function selectAll(table, checked) {
            ...
            for (var i = 0, len = inputs.length; i < len; i++) {
                var input = inputs[i];
                if (input.id.indexOf(cbIdPrefix) >= 0 && !input.disabled) {
