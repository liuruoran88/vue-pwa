...
lhgcalendar.fn = lhgcalendar.prototype =
{
	...
	_init: function( config )
	{
		...
		inpVal = $.trim( that.inpE.value ) || config.defaultValue || '';

		if( inpVal.length > 0 )
		    date = getDate(convertDate(inpVal, 'yyyy-MM-dd HH:mm:ss'), 'yyyy-MM-dd HH:mm:ss');
		else
		    date = new Date();
	...
	_getDOM: function()
	{
		...
		wrap.style.cssText = 'background:#fff;border:1px solid #ccc;position:absolute;display:none;z-index:' + this.config.zIndex + ';';
	...
	_setDate: function( day )
	{
		...
		var that = this, opt = that.config, DOM = that.DOM,
		    date = tmpDate = new Date( that.year, that.month-1, day );
		...
		that.day = day;
		if (that.config.maxDate) {
			var max = that.config.maxDate.match(/\d+/g);
			var maxYear = parseInt(max[0], 10);
			var maxMonth = parseInt(max[1], 10);
			var maxDay = parseInt(max[2], 10);
			if (date.getTime() > new Date(maxYear, maxMonth - 1, maxDay).getTime()) {
				return;
			}
		}
		if (that.config.minDate) {
			var min = that.config.minDate.match(/\d+/g);
			var minYear = parseInt(min[0], 10);
			var minMonth = parseInt(min[1], 10);
			var minDay = parseInt(min[2], 10);
			if (date.getTime() < new Date(minYear, minMonth - 1, minDay).getTime()) {
				return;
			}
		}
	...
	_addEvent: function()
	{
		...
		DOM.wrap.bind('click',function(evt){
			...
			if( target[expando+'D'] ) {
				that.day = parseInt(target[expando+'D'], 10);
				$('.cui_db td').removeClass('cui_today');
				$(target).parent().addClass('cui_today');
			}
